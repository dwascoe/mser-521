...
async function createChart() {
  ...
  snowDays1 = serverData.map(row => row['Average Daily Steps per Month in 2023']);
  snowDays2 = serverData.map(row => row['Average Daily Steps per Month in 2024']);
  ...
  const options = {
    chart: {
     ...
     title: {
    text: 'Average Steps per Month by Daniel in 2023-2024',
    align: 'center',
    },
    colors: ['green', '#2847d2ff'], // series 1 and series 2 colors
    plotOptions: {
      bar: {
        horizontal: false,
      },
    ...
    series: [
      { name: 'Average Daily Steps per month in 2023', data: snowDays1 },
      { name: 'Average Daily Steps per month in 2024', data: snowDays2 },
    ],
